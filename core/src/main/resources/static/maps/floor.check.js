...
let floorBuildingName = '愿由щ룞';
let floorName = '1층;
let floorIsOutdoor = false;
...
async function init() {
    ...
    try {
        ...
        if (accountBtn && currentUser) {
            accountBtn.textContent = currentUser.username || 'User';
        }
        if (isAdmin) {
...
function resolvePlanImagePath(buildingName, floorNameText){
    const b = normalizeName(buildingName);
    const f = normalizeName(floorNameText);
    if (b.includes('蹂듭?愿')) {
        if (f.includes('吏??) || f.includes('b1')) return '/images/bokji_B1.png';
    ...
    if (b.includes('愿由щ룞') && f.includes('1')) return '/images/gwanri_1F.png';
    return '';
}

function applyFloorContextFromQuery(){
    const qs = new URLSearchParams(window.location.search || '');
    floorBuildingName = (qs.get('buildingName') || qs.get('building') || '愿由щ룞').trim() || '愿由щ룞';
    floorName = (qs.get('floorName') || qs.get('floor') || '1층).trim() || '1층;
    floorIsOutdoor = normalizeName(floorBuildingName).includes('옥외') || normalizeName(floorName).includes('옥외');

    if (infoTitle) infoTitle.textContent = floorBuildingName;
    if (infoSub) infoSub.textContent = floorName;

    if (floorIsOutdoor) {
        if (mapImg) mapImg.src = '/images/drone_photo.JPG';
        if (zoneSvg) zoneSvg.style.display = '';
        return;
    }

    const img = resolvePlanImagePath(floorBuildingName, floorName);
    if (mapImg) {
        mapImg.src = img || '';
        mapImg.alt = img ? `${floorBuildingName} ${floorName} 도면` : '도면 ?놁쓬';
    }
...
async function loadIndoorMarkers(){
    if (floorIsOutdoor || !outdoorMarkers) return;
    try {
        const [extRes, hydRes] = await Promise.all([
            apiFetch('/fire-api/extinguishers?size=1000&page=0'),
            apiFetch('/fire-api/hydrants?size=1000&page=0')
        ]);
        if (!extRes || !hydRes) return;

        const extJson = await extRes.json();
        const hydJson = await hydRes.json();
        const extList = extJson?.data?.content || [];
        const hydList = hydJson?.data?.content || [];

        const bNorm = normalizeName(floorBuildingName);
        const fNorm = normalizeName(floorName);
        const samePlace = (item) =>
            normalizeName(item?.buildingName) === bNorm &&
            normalizeName(item?.floorName) === fNorm &&
            item?.x != null && item?.y != null;

        const markers = [];
        extList.filter(samePlace).forEach(e => {
            markers.push({
                key: `E-${e.extinguisherId}`,
                x: Number(e.x),
                y: Number(e.y),
                icon: '/images/Extinguisher.PNG'
            });
        });
        hydList
            .filter(h => (h?.hydrantType || '').toLowerCase() !== 'outdoor')
            .filter(samePlace)
            .forEach(h => {
                markers.push({
                    key: `H-${h.hydrantId}`,
                    x: Number(h.x),
                    y: Number(h.y),
                    icon: '/images/indoorfirehydrant.PNG'
                });
            });
        ...
        markers.forEach(m => {
            ...
            if (coverFrame) {
                const px = coverFrame.ox + (coverFrame.w * (m.x / 100));
                const py = coverFrame.oy + (coverFrame.h * (m.y / 100));
            ...
            btn.innerHTML = `<img class="icon" src="${m.icon}" alt="">`;
            outdoorMarkers.appendChild(btn);
        });
    } catch (e) {
        console.error('indoor marker load error', e);
    }
}
...
// 甕곌쑵????源??btnOutdoorAdd?.addEventListener('click', ()=>{
btnOutdoorAdd?.addEventListener('click', ()=>{
    if(!isAdmin){ alert('愿由ъ옄留?추가?????덉뒿?덈떎.'); return; }
    setOutdoorToolMode('add');
    setOutdoorHint('지점의 추가???꾩튂瑜??대┃?섏꽭??');
});
