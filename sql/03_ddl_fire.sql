...
CREATE TABLE IF NOT EXISTS extinguisher (
    ...
    install_date            DATE            NOT NULL                COMMENT '제조일 (기존 InstallDate)',
    replacement_cycle_years INT             NOT NULL DEFAULT 5      COMMENT '교체 주기(년)',
...
SELECT
    ...
    latest.inspected_by_name AS last_inspector_name,
    latest.is_faulty        AS last_is_faulty,
    latest.fault_reason     AS last_fault_reason
FROM extinguisher e
JOIN building b ON e.building_id = b.building_id
JOIN floor f    ON e.floor_id    = f.floor_id
LEFT JOIN LATERAL (
    SELECT inspection_date, inspected_by_name, is_faulty, fault_reason
    FROM extinguisher_inspection
    WHERE extinguisher_id = e.extinguisher_id
